...
export default function IngredientModal({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { errorMessage } = await (isEditing
      ? updateIngredientAction(ingredient.id, data)
      : createIngredientAction(data));
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              type="date"
              defaultValue={
                (isEditing ? ingredient.dateBought : new Date())
                  .toISOString()
                  .split("T")[0]
              }
              className="w-full rounded border px-2 py-1"
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              type="checkbox"
              id="isPackaged"
              defaultChecked={ingredient?.isPackaged ?? false}
            />
            <label htmlFor="isPackaged" className="text-sm font-medium">
              Is Packaged
            </label>
          </div>
